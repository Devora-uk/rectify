...
interface NavigationProps { activePage?: 'home' | 'about' | 'services' | 'insights' | 'areas' | 'partner' | 'contact'; overlay?: boolean }
const links = [
...
export default function Navigation({ activePage = 'home', overlay = false }: NavigationProps) {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 border-b transition-[background-color,border-color,box-shadow] duration-200 ${scrolled || open ? 'border-[#dce8fa] bg-white/95 shadow-[0_10px_40px_rgba(2,20,91,.06)] backdrop-blur-xl' : overlay ? 'border-transparent bg-transparent' : 'border-[#e5edf9] bg-white/95 backdrop-blur-md'}`}>
    <div className="section-shell flex h-[78px] items-center justify-between">
      ...
      <nav className={`hidden items-center lg:flex ${floating ? 'gap-1.5 rounded-full border border-white/20 bg-[#020d3b]/30 p-1.5 backdrop-blur-md' : 'gap-4 xl:gap-6'}`} aria-label="Main navigation">
      ...
      <button type="button" className={`grid h-11 w-11 place-items-center rounded-full border transition-colors lg:hidden ${floating?'border-white/25 bg-[#020d3b]/25 text-white backdrop-blur':'border-[#cddbf2] bg-white/80 text-[#041353]'}`} onClick={() => setOpen(current => !current)} aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? 'Close navigation' : 'Open navigation'}>{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
    </div>
    {open && <nav id="mobile-navigation" className="max-h-[calc(100dvh-78px)] overflow-y-auto border-t border-[#e1eafb] bg-white px-5 pb-8 pt-2 shadow-[0_24px_60px_rgba(2,20,91,.12)] lg:hidden" aria-label="Mobile navigation">
